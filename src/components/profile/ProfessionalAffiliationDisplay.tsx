...
import { PaperClipIcon, DocumentTextIcon, CalendarDaysIcon, UserGroupIcon, BuildingOfficeIcon, EyeIcon } from '@heroicons/react/24/outline'; // Added EyeIcon
...
export default function ProfessionalAffiliationDisplay({ item, isEditing, stagedFile }: Props) {
    ...
    const getFileName = (url: string | null | undefined): string => {
        if (!url) return 'Membership Proof';
        try {
            return decodeURIComponent(url.split('/').pop() || 'Membership Proof');
        } catch {
            return 'Membership Proof';
        }
    };

    const proofFileName = getFileName(item.membershipProofUrl);
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            {item.membershipProofUrl && proofFileName && (
                <FilePreviewModal
                    isOpen={isPreviewModalOpen}
                    onClose={closePreview}
                    fileUrl={item.membershipProofUrl}

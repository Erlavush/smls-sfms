...
import {
    ...
    CheckIcon, XMarkIcon, InformationCircleIcon, ExclamationTriangleIcon, ClockIcon, CheckBadgeIcon
...
// --- Helper Functions (Keep existing helpers like formatDate) ---
...
export default function ProfilePage() {
    ...
    // --- Fetch data (Keep existing fetch logic) ---
    ...
    // --- Edit Mode Toggles (Keep existing logic) ---
    ...
    // --- Save Changes (Keep existing logic) ---
    ...
    // --- Category/Item Local Edit Handlers (Keep existing logic) ---
    ...
    // --- Determine data source based on edit state ---
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen"> {/* Added responsive padding */}
            {/* === Page Header & Actions === */}
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 pb-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                        ...
                        disabled={isPending}
                        className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-70 ${
                            isEditing
                            ? 'bg-gray-600 hover:bg-gray-700 focus:ring-gray-500' // Cancel style
                            : 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-600' // Edit style
                        }`}>
                        {isEditing ? (
                    ...
                    {isEditing && (
                        <button
                            ...
                            disabled={isPending || editingItemId !== null} // Disable if pending or an item is being edited inline
                    ...
                    <div className="relative">
                        <button
                            ...
                            disabled={isPending || isEditing} // Disable in edit mode too
                            className="inline-flex items-center gap-1 rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed"
                            aria-haspopup="true"
                ...
                {isEditing && editingItemId !== null && (
                    <p className="mt-2 text-xs text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200 w-full flex items-center gap-1">
                       <InformationCircleIcon className='h-4 w-4 flex-shrink-0'/>
                       Finish editing the current item (<CheckIcon className='inline h-3 w-3 text-blue-600'/> <XMarkIcon className='inline h-3 w-3 text-gray-600'/>) before saving all profile changes.
                    </p>
                )}
            </div>
            ...
            <div className="mb-8 rounded-lg border border-gray-200 bg-white shadow-md overflow-hidden">
                 ...
                 <div className="bg-sky-50 border-b border-sky-100 px-4 py-3">
                     <h2 className="text-lg font-semibold text-sky-800">Basic Information</h2>
                 </div>
                 {/* Card Body */}
                 <div className="p-4">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                {Array.from(visibleCategories).map(categoryKey => {
                    ...
                    const isCategoryEditable = true; // Assuming all forms exist
                        ...
                        <div key={categoryKey} className="flex flex-col rounded-lg border border-gray-200 bg-white shadow-md overflow-hidden">
                            ...
                            <div className="flex items-center justify-between border-b border-sky-100 bg-sky-50 px-4 py-3">
                                <div className="flex items-center gap-2">
                                    <CategoryIcon className="h-5 w-5 text-sky-700" aria-hidden="true" />
                                    <h2 className="text-base font-semibold text-sky-800">{categoryMeta.title}</h2>
                                </div>
                                {/* Add Button - only show in edit mode */}
                                {isEditing && (
                                    <button
                                        onClick={() => handleAddItemLocally(categoryKey)}
                                        className="rounded-full bg-indigo-100 p-1 text-indigo-600 hover:bg-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
                            ...
                            <div className="flex-grow p-4 text-sm text-gray-700">
                                {categoryData.length === 0 ? (
                                    <p className="italic text-gray-400 text-center py-4">No items recorded for this section.</p>
                                ) : (
                                    <ul className="space-y-4">
                                        {categoryData.map((item: EditableItem, index: number) => (
                                            <li key={item.id || index} className={`border-b border-gray-100 pb-4 last:border-b-0 last:pb-0 relative group ${
                                                isEditing && item.id === editingItemId ? 'bg-blue-50 p-3 rounded-md border border-dashed border-blue-300 shadow-inner -m-3' // Apply negative margin to counter padding
                                                : isNewItem(item) ? 'bg-green-50 p-3 rounded-md border border-dashed border-green-300 shadow-inner -m-3' // Apply negative margin
                                                : ''
                                            }`}>
                                                ...
                                                {isEditing && isCategoryEditable && (isNewItem(item) || item.id === editingItemId) ? (
                                                    ...
                                                    <>
                                                        ...
                                                        {/* Item specific Save/Cancel Buttons for EXISTING items being edited */}
                                                        {!isNewItem(item) && item.id === editingItemId && (
                                                            <div className='flex justify-end gap-2 mt-3'>
                                                                <button onClick={() => handleCancelItemEdit(categoryKey, item.id)} disabled={isPending} className='inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-gray-200 hover:bg-gray-300 text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400'>
                                                                    <XMarkIcon className='h-3 w-3'/> Cancel
                                                                </button>
                                                                <button onClick={() => handleSaveEditedItem(item.id)} disabled={isPending} className='inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-blue-100 hover:bg-blue-200 text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400'>
                                                                    <CheckIcon className='h-3 w-3'/> Save Item
                                                                </button>
                                                            </div>
                                                        )}
                                                         ...
                                                         {isNewItem(item) && (
                                                            <div className="text-right mt-2">
                                                                <button onClick={() => handleDeleteItemLocally(categoryKey, item.id)} className="inline-flex items-center gap-1 text-xs text-red-600 hover:text-red-800 disabled:opacity-50" disabled={isPending} title="Delete this new item">
                                                ...
                                                {isEditing && item.id !== editingItemId && !isNewItem(item) && (
                                                    <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                        {isCategoryEditable && (
                                                            <button
                                                                onClick={() => handleStartEditingItem(categoryKey, item.id)}
                                                                className="p-1 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 focus:outline-none focus:ring-1 focus:ring-blue-400 focus:ring-offset-1"
                                                                title={`Edit`}
                                                                disabled={isPending || editingItemId !== null} // Disable if another item is being edited
                                                            >
                                                                <PencilIcon className="h-3.5 w-3.5" />
                                                            </button>
                                                        )}
                                                        <button
                                                            onClick={() => handleDeleteItemLocally(categoryKey, item.id)}
                                                            className="p-1 rounded-full bg-red-100 text-red-600 hover:bg-red-200 focus:outline-none focus:ring-1 focus:ring-red-400 focus:ring-offset-1"
                                                            title={`Delete`}
                                                            disabled={isPending || editingItemId !== null} // Disable if another item is being edited
                                                        >
